  ...
  Val.prototype.validate = function (schema, cb) {
    ...
    return function validate(req, res, next) {
      ...
      for (var i in schema) {
        ...
        //
        // required parameter or body
        //
        if (def.required) {
          //
          // an example would be `email`, or `body`, or a non simple validation
          //
          ...
          if (basic_types[prop]) {
            ...
            //
            // if body is required we just need to check it is there
            //
            if (!req[self.body]) {
              cb(new Error(['We could not find a body in your request and it is required.',
                ' Please do try again but this time providing a body.'
              ].join('')), req, res, next);
              return;
            }

            //
            // we have a body so it's all cool
            //
            next();
          }
          //
